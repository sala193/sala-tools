...
- 一張卡片、一個計算機、三個分頁，由 `MortgageTool.astro` 組合（分頁切換、按鈕帶值）：**月付金試算**（`LoanPanel.astro`：貸款金額 → 月付）、**月付反推房價**（`ReversePanel.astro`：每月願意繳多少＋自備 2/3/4 成 → 可買多少錢，房價＝月付換算的貸款 ÷ 貸款成數，並列出各成數對照；不放稅費雜支——契稅、代書、仲介、設定費因物件而異，由蔡莎拉當面說明、簽約前確認）、**購屋能力**（`AffordPanel.astro`：月收入＋自備款 → 買得起多少）
