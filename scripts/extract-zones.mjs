...
const COMMUNITIES = [
  ['森晴', '森聯之王森晴', 1380, 385], ['森朵', '森聯之王森朵', 1375, 570], ['森悅', '森聯之王森悅', 1467, 645],
  ['森闊', '森聯之王森闊', 1472, 983], ['森藏', '森聯之王森藏', 1517, 920], ['森耀', '森聯LIFE森耀', 1302, 393],
  ['森睦', '森聯LIFE森睦', 568, 797], ['雙捷A+', '雙捷A+', 1467, 795], ['豐邑日日', '豐邑日日', 535, 348],
  ...
  ['立瑾way', null, 517, 285], ['立瑾綻', null, 443, 290], ['僑駿響', null, 490, 415], ['幸福莊園NO2', null, 510, 470],
  ['京澄為泰', null, 907, 883], ['合康建設', null, 993, 865], ['森朗', null, 1012, 887], ['富御昕', null, 1087, 872],
  ['定泰公園翫', null, 1255, 300], ['合康世紀滙', null, 1262, 605], ['梧桐莊園', null, 910, 1013], ['鶯桃小城', null, 1025, 1043],
  ['鳳鳴園', null, 1000, 1142], ['金合昌', null, 658, 838],
  ['景程禾雅', null, 947, 712], ['和瑞微美', null, 1050, 777],
];

// 圖上是建築照片或圖示、不是單色的基地，抓不到可靠範圍：不畫外框，只保留點位
const NO_PARCEL = new Set(['森晴', '森朵', '森悅', '森闊', '森藏', '青松翫', '微笑海悅2', '金和昌仰沐', '森朗', '合康建設', '鳳鳴尊邸']);
// 字很大的社區，補洞半徑要大一點
...
const SPLITS = [
  ...
  { seed: '丞石菁英薈3', names: ['丞石菁英薈3', '森耀'], axis: 'x', at: 1277 },
];
...
// 地圖上的名稱與 OSM 原名（osmName）都能查到
const byName = {};
for (const p of poi.points) if (p.cat === 'community') { byName[p.name] = p; if (p.osmName) byName[p.osmName] = p; }
